                    ...
                    if($userpass === $password) {
                        if ($amount > $payee_balance) {
                            array_push($errors, "Insufficient Balance");
                                ...
                                VALUES (NULL, NULL, NOW(), 'Payment Transferred to Globe', '$amount', '0', '$payee_balance_final')";
                                    $date = date("Y-m-d H:i:s");
                                    $email = $user_email;
                                    $subject = "Gentlemen's National Bank Notification | Bills Receipt Globe";
                                    ...
                                    mail($email, $subject, $message, $sender);
                                        ...
                                        VALUES ('$username', '$accountnumber', 
                                        'Transferred an amount of $amount to Globe', NOW())";
                                            mysqli_query($db, $query_logfile);
                                            mysqli_query($db, $payee_update_query);
                                            $_SESSION['globe_bills_successful'] = true;
                                                header('location: globe_bills_successful.php');
                        }
                    }
                    else {
